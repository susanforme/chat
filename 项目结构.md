...
| √    | /history/:id | get    |            | {status:1,data:{history:send:{userName,headImg,id},receive:{},msg:string,createTime:string}[],user:{}[]} | id为两个人id拼接而成以_为间隔                     |
...
| √    | /order               | post   | {commodityId,receive:{area,phoneNum,},buyerId,sellerId,evaluate?} | 提交订单后会自动校验余额                                     |
